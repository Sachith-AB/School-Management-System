...
export default function Home() {
  ...
  const [deleteStudentd,setDeleteStudentId]=useState(' ')
  ...
  const handleDelete=async()=>{
    setShowModal(false);
    try{
      const res = await axios.delete(`http://localhost:4000/api/student/delete/${deleteStudentd}`);
      setStudents((prev)=>prev.filter((student)=>student._id !== deleteStudentd))
    ...
    <div className=''>
      <div className='flex flex-col p-3 dark:bg-slate-800 gap-4 md:w-72
        ...
        <div className='flex flex-col w-full shadow-md p-2 rounded-md dark:bg-gray-800'>
            ...
            {students.length>0?( <Table hoverable>
                    <Table.Head>
